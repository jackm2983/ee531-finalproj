module trade_engine_top #(
  ...
  // Incoming ticks (from TB or from a UART/streaming bridge)
  ...
  logic [LANES-1:0] lane_in_valid, lane_in_ready;
  logic [LANES-1:0][PAY_W-1:0] lane_in_data;
  ...
  symbol_router #(.LANES(LANES), .PRICE_W(PRICE_W), .SIZE_W(SIZE_W), .SEQ_W(SEQ_W)) u_router (
    ...
    .in_size(p_size),
    .in_seq(p_seq),
    .lane_valid(lane_in_valid),
    .lane_ready(lane_in_ready),
    .lane_data(lane_in_data)
  );
  ...
  generate
    for (g=0; g<LANES; g++) begin : GEN_LANES
      lane_engine #(
        ...
        .clk(clk), .rst_n(rst_n),
        .in_valid(lane_in_valid[g]),
        .in_ready(lane_in_ready[g]),
        .in_data(lane_in_data[g]),
  ...
  trigger_arbiter #(.LANES(LANES), .TRIG_W(TRIG_W)) u_arb (
    .clk(clk), .rst_n(rst_n),
