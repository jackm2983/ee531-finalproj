module trigger_arbiter #(
  ...
  parameter int TRIG_W = 8+1+24+16  // must match lane out width
)(
  input  logic clk,
  input  logic rst_n,
  ...
  input  logic [LANES-1:0][TRIG_W-1:0] in_data,

  output logic out_valid,
  input  logic out_ready,
  output logic [TRIG_W-1:0] out_data
);
  ...
  always_comb begin
    ...
    if (found) in_ready[sel] = out_ready; // only selected lane pops
  end
